import { ActionRowBuilder, Client, ButtonBuilder, ButtonStyle, MessageFlags, ModalBuilder, TextInputBuilder, TextInputStyle, ModalActionRowComponent, InteractionCallbackResponse } from "discord.js";
...
type RunMap = Map<number, Run>

let runid = 0 //Initialize runid
let runmap: RunMap = new Map() //Stores a map of all active runs
...
client.on("interactionCreate", async (interaction) => {
    if (interaction.isChatInputCommand()) {
        if (interaction.commandName === "host") {
            ...
            const pw = new TextInputBuilder()
                ...
                .setLabel("Password")
                .setStyle(TextInputStyle.Short)
            const firstrow: ActionRowBuilder<TextInputBuilder> = new ActionRowBuilder<TextInputBuilder>().addComponents(game)
            const secondrow: ActionRowBuilder<TextInputBuilder> = new ActionRowBuilder<TextInputBuilder>().addComponents(pw)
    ...
    if (interaction.isButton()) {
        ...
        if (!run) {
            await interaction.reply({ content: `Could not find run info. Error: ${HostbotError.BUTTON_INTERACTION_NO_ID}`, flags: MessageFlags.Ephemeral });
            return;
        }
        ...
        if (action === "join") {
            ...
            if (run.runners.includes(interaction.user.id)) {
                ...
                runmap.set(id, run)
                await interaction.reply({ content: `You left <@${run.host}>'s ${run.zone} runs.`, flags: MessageFlags.Ephemeral });
    ...
    if (interaction.isModalSubmit()) {
        ...
        let run = runmap.get(id)
        if (!run) {
            await interaction.followUp({ content: `No run found. Error Code: ${HostbotError.NO_MODAL_ID}`, flags: MessageFlags.Ephemeral });
            return;
        }
        let zoneName = {
            tombs: "Tombs",
